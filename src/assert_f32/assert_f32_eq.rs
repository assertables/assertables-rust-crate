...
mod test_assert_f32_eq_as_result {
    ...
    fn eq() {
        ...
        let b: f32 = EQ;
        for _ in 0..1 {
            let actual = assert_f32_eq_as_result!(a, b);
    ...
    fn eq_once() {
        ...
        let result = assert_f32_eq_as_result!(a(), b());
        assert!(result.is_ok());
...
macro_rules! assert_f32_eq {
    ($a:expr, $b:expr $(,)?) => {
        match $crate::assert_f32_eq_as_result!($a, $b) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    };
...
macro_rules! debug_assert_f32_eq {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_f32_eq!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_f32_eq {
    use crate::assert_f32::EQ;
    use std::panic;
    ...
    fn eq() {
        ...
        let b: f32 = EQ;
        for _ in 0..1 {
            let _actual = debug_assert_f32_eq!(a, b);
            // assert_eq!(actual, ());
        }
    }
}
