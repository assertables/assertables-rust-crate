...
#[cfg(test)]
mod test_assert_count_gt_x_as_result {
    use std::sync::Once;
    ...
    fn gt() {
        ...
        let b = 1;
        for _ in 0..1 {
            let actual = assert_count_gt_x_as_result!(a, b);
    ...
    fn gt_once() {
        ...
        let result = assert_count_gt_x_as_result!(a(), b());
        assert!(result.is_ok());
...
mod test_assert_count_gt_x {
    ...
    fn gt() {
        ...
        let b = 1;
        for _ in 0..1 {
            let actual = assert_count_gt_x!(a, b);
    ...
    fn eq() {
        let result = panic::catch_unwind(|| {
            let a = "x".chars();
            let b = 1;
            let _actual = assert_count_gt_x!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_count_gt_x!(a, b)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_count_gt_x.html\n",
...
macro_rules! debug_assert_count_gt_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_count_gt_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_count_gt_x {
    use std::panic;
    ...
    fn gt() {
        ...
        let b = 1;
        for _ in 0..1 {
            let _actual = debug_assert_count_gt_x!(a, b);
            // assert_eq!(actual, (2, 1));
        }
    }
