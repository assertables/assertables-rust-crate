...
macro_rules! assert_fn_eq_x {
    ...
    ($a_function:path, $b_expr:expr $(,)?) => {
        match $crate::assert_fn_eq_x_as_result!($a_function, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    };
...
mod test_assert_fn_eq_x {
    ...
    mod arity_1 {
        ...
        fn eq() {
            ...
            let b: i8 = 1;
            for _ in 0..1 {
                let actual = assert_fn_eq_x!(f, a, b);
                let expect = 1;
        ...
        fn failure() {
            let result = panic::catch_unwind(|| {
                let a: i8 = 1;
                let b: i8 = 2;
                let _actual = assert_fn_eq_x!(f, a, b);
            });
            let message = concat!(
                "assertion failed: `assert_fn_eq_x!(a_function, a_param, b_expr)`\n",
                "https://docs.rs/assertables/10.1.0/assertables/macro.assert_fn_eq_x.html\n",
    ...
    mod arity_0 {
        ...
        fn ne() {
            let result = panic::catch_unwind(|| {
                let b: i8 = 2;
                let _actual = assert_fn_eq_x!(f, b);
            });
            let message = concat!(
                "assertion failed: `assert_fn_eq_x!(a_function, b_expr)`\n",
                "https://docs.rs/assertables/10.1.0/assertables/macro.assert_fn_eq_x.html\n",
...
macro_rules! debug_assert_fn_eq_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_fn_eq_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_fn_eq_x {
    use std::panic;
    ...
    mod arity_1 {
        ...
        fn eq() {
            ...
            let b: i8 = 1;
            for _ in 0..1 {
                let _actual = debug_assert_fn_eq_x!(f, a, b);
                let _expect = 1;
                // assert_eq!(actual, expect);
            }
        }
    ...
    mod arity_0 {
        ...
        fn eq() {
            let b: i8 = 1;
            for _ in 0..1 {
                let _actual = debug_assert_fn_eq_x!(f, b);
                let _expect = 1;
                // assert_eq!(actual, expect);
            }
        }
