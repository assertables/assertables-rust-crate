...
#[cfg(test)]
mod test_assert_diff_ne_x_as_result {
    use std::sync::Once;
    ...
    fn lt_once() {
        ...
        let result = assert_diff_ne_x_as_result!(a(), b(), x());
        assert!(result.is_ok());
...
macro_rules! assert_diff_ne_x {
    ($a:expr, $b:expr, $x:expr $(,)?) => {
        match $crate::assert_diff_ne_x_as_result!($a, $b, $x) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    };
...
mod test_assert_diff_ne_x {
    ...
    fn eq() {
        ...
        let x: i8 = 3;
        let result = panic::catch_unwind(|| {
            let _actual = assert_diff_ne_x!(a, b, x);
        });
        let message = concat!(
            "assertion failed: `assert_diff_ne_x!(a, b, x)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_diff_ne_x.html\n",
