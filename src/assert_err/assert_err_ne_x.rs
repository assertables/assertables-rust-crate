...
#[cfg(test)]
mod test_assert_err_ne_x_as_result {
    use std::sync::Once;
    ...
    fn lt_once() {
        ...
        let result = assert_err_ne_x_as_result!(a(), b());
        assert!(result.is_ok());
...
mod test_assert_err_ne_x {
    ...
    fn success() {
        ...
        let b: i8 = 2;
        for _ in 0..1 {
            let actual = assert_err_ne_x!(a, b);
            assert_eq!(actual, 1);
        }
    }
    ...
    fn ne() {
        let result = panic::catch_unwind(|| {
            let a: Result<i8, i8> = Err(1);
            let b: i8 = 1;
            let _actual = assert_err_ne_x!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_err_ne_x!(a, b)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_err_ne_x.html\n",
...
macro_rules! debug_assert_err_ne_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_err_ne_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_err_ne_x {
    use std::panic;
    ...
    fn success() {
        ...
        let b: i8 = 2;
        for _ in 0..1 {
            let _actual = debug_assert_err_ne_x!(a, b);
            // assert_eq!(actual, 1);
        }
    }
