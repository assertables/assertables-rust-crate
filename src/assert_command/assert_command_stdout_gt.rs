...
mod test_assert_command_stdout_gt_as_result {
    ...
    fn gt_once() {
        ...
        let result = assert_command_stdout_gt_as_result!(a(), b());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_command_stdout_gt {
    use std::panic;
    use std::process::Command;
    ...
    fn eq() {
        let result = panic::catch_unwind(|| {
            ...
            let mut b = Command::new("bin/printf-stdout");
            b.args(["%s", "alfa"]);
            let _actual = assert_command_stdout_gt!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_command_stdout_gt!(a_command, b_command)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_command_stdout_gt.html\n",
...
macro_rules! debug_assert_command_stdout_gt {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_command_stdout_gt!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_command_stdout_gt {
    use std::panic;
    use std::process::Command;
    ...
    fn gt() {
        ...
        b.args(["%s", "aa"]);
        for _ in 0..1 {
            let _actual = debug_assert_command_stdout_gt!(a, b);
            // assert_eq!(actual, (vec![b'a', b'l', b'f', b'a'], vec![b'a', b'a']));
        }
    }
