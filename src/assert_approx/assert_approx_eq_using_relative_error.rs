//! Assert a number is approximately equal to as another by using relative error epsilon.
//!
//! Pseudocode:<br>
//! | a - b | ≤ ε * min(a, b)
//!
...
//! assert_approx_eq_using_relative_error.rs!(a, b, epsilon);
//! ```
//!
//! ## Comparisons
//!
...
//!   by the magnitude of the exact value. This can be used to compare
...
//!   error divided by the magnitude of the exact value. This is typically useful
...
//!   creating and maintaining `assert_in_delta` and `assert_approx_eq_using_relative_error` code.
//!
//! # Module macros
//!
//! * [`assert_approx_eq_using_relative_error`](macro@crate::assert_approx_eq_using_relative_error)
//! * [`assert_approx_eq_using_relative_error_as_result`](macro@crate::assert_approx_eq_using_relative_error_as_result)
//! * [`debug_assert_approx_eq_using_relative_error`](macro@crate::debug_assert_approx_eq_using_relative_error)
...
///
#[macro_export]
macro_rules! assert_approx_eq_using_relative_error_as_result {
        ...
        match (&$a, &$b, &$epsilon) {
            (a, b, epsilon) => {
                ...
                if abs_diff <= rhs {
                    ...
                    Err(format!(
                        concat!(
                            "assertion failed: `assert_approx_eq_using_relative_error!(a, b, ε)`\n",
                            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_approx_eq_using_relative_error.html\n",
                            "                   a label: `{}`,\n",
                            "                   a debug: `{:?}`,\n",
                            "                   b label: `{}`,\n",
                            "                   b debug: `{:?}`,\n",
...
#[cfg(test)]
mod test_assert_approx_eq_using_relative_error_as_result {
    use std::sync::Once;
    ...
    fn success() {
        ...
        let epsilon: f64 = 0.03;
        for _ in 0..1 {
            let actual = assert_approx_eq_using_relative_error_as_result!(a, b, epsilon);
    ...
    fn success_once() {
        ...
        let result = assert_approx_eq_using_relative_error_as_result!(a(), b(), epsilon());
        assert!(result.is_ok());
...
/// let b: f64 = 103.0;
/// let epsilon: f64 = 0.03;
/// assert_approx_eq_using_relative_error!(a, b, epsilon);
///
...
/// assert_approx_eq_using_relative_error!(a, b, epsilon);
/// # });
/// // assertion failed: `assert_approx_eq_using_relative_error!(a, b, epsilon)`
/// // https://docs.rs/assertables/…/assertables/macro.assert_approx_eq_using_relative_error.html
/// //                    a label: `a`,
/// //                    a debug: `100.0`,
/// //                    b label: `b`,
/// //                    b debug: `103.0`,
...
///
#[macro_export]
macro_rules! assert_approx_eq_using_relative_error {
    ($a:expr, $b:expr, $epsilon:expr $(,)?) => {
        match $crate::assert_approx_eq_using_relative_error_as_result!($a, $b, $epsilon) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    };
...
#[cfg(test)]
mod test_assert_approx_eq_using_relative_error {
    use std::panic;
    ...
    fn success() {
        ...
        let epsilon: f64 = 0.03;
        for _ in 0..1 {
            let actual = assert_approx_eq_using_relative_error!(a, b, epsilon);
    ...
    fn failure() {
        ...
        let epsilon: f64 = 0.02;
        let result = panic::catch_unwind(|| {
            let _actual = assert_approx_eq_using_relative_error!(a, b, epsilon);
        });
        let message = concat!(
            "assertion failed: `assert_approx_eq_using_relative_error!(a, b, ε)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_approx_eq_using_relative_error.html\n",
...
/// | a - b | ≤ ε * min(a, b)
///
/// This macro provides the same statements as [`assert_approx_eq_using_relative_error`](macro.assert_approx_eq_using_relative_error.html),
...
///
#[macro_export]
macro_rules! debug_assert_approx_eq_using_relative_error {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_approx_eq_using_relative_error!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_approx_eq_using_relative_error {
    use std::panic;
    ...
    fn success() {
        ...
        let epsilon: f64 = 0.03;
        for _ in 0..1 {
            let _actual = debug_assert_approx_eq_using_relative_error!(a, b, epsilon);
