...
#[cfg(test)]
mod test_assert_fs_read_to_string_le_x_as_result {
    ...
    fn lt() {
        ...
        let x = "bravo\n";
        for _ in 0..1 {
            let actual = assert_fs_read_to_string_le_x_as_result!(path, x);
    ...
    fn gt_once() {
        ...
        let result = assert_fs_read_to_string_le_x_as_result!(a(), b());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_fs_read_to_string_le_x {
    #[allow(unused_imports)]
    use std::io::Read;
    use std::panic;
    use std::path::PathBuf;
    ...
    fn lt() {
        ...
        let x = "bravo\n";
        for _ in 0..1 {
            let actual = assert_fs_read_to_string_le_x!(path, x);
    ...
    fn gt() {
        ...
        let x = "alfa\n";
        let result = panic::catch_unwind(|| {
            let _actual = assert_fs_read_to_string_le_x!(path, x);
        });
        let message = format!(
            concat!(
                "assertion failed: `assert_fs_read_to_string_le_x!(a_path, b_expr)`\n",
...
#[cfg(test)]
mod test_debug_assert_fs_read_to_string_le_x {
    #[allow(unused_imports)]
    use std::io::Read;
    use std::panic;
    use std::path::PathBuf;
    ...
    fn lt() {
        ...
        let x = "bravo\n";
        for _ in 0..1 {
            let _actual = debug_assert_fs_read_to_string_le_x!(path, x);
            // assert_eq!(actual, String::from("alfa\n"));
        }
    }
