...
#[cfg(test)]
mod test_assert_fs_read_to_string_eq_as_result {
    ...
    fn eq() {
        ...
        let actual = assert_fs_read_to_string_eq_as_result!(a, b);
        assert_eq!(
            actual.unwrap(),
            (String::from("alfa\n"), String::from("alfa\n"))
        );
...
#[cfg(test)]
mod test_assert_fs_read_to_string_eq {
    #[allow(unused_imports)]
    use std::io::Read;
    use std::panic;
    use std::path::PathBuf;
    ...
    fn eq() {
        ...
        let b = DIR.join("alfa.txt");
        for _ in 0..1 {
            let actual = assert_fs_read_to_string_eq!(a, b);
    ...
    fn lt() {
        ...
        let b = DIR.join("bravo.txt");
        let result = panic::catch_unwind(|| {
            let _actual = assert_fs_read_to_string_eq!(a, b);
        });
        let message = format!(
            concat!(
                "assertion failed: `assert_fs_read_to_string_eq!(a_path, b_path)`\n",
