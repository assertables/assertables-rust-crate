...
#[cfg(test)]
mod test_assert_fs_read_to_string_le_as_result {
    ...
    fn lt() {
        ...
        let actual = assert_fs_read_to_string_le_as_result!(a, b);
        assert_eq!(
            actual.unwrap(),
            (String::from("alfa\n"), String::from("bravo\n"))
        );
...
#[cfg(test)]
mod test_assert_fs_read_to_string_le {
    #[allow(unused_imports)]
    use std::io::Read;
    use std::panic;
    use std::path::PathBuf;
    ...
    fn lt() {
        ...
        let b = DIR.join("bravo.txt");
        for _ in 0..1 {
            let actual = assert_fs_read_to_string_le!(a, b);
    ...
    fn gt() {
        ...
        let b = DIR.join("alfa.txt");
        let result = panic::catch_unwind(|| {
            let _actual = assert_fs_read_to_string_le!(a, b);
        });
        let message = format!(
            concat!(
                "assertion failed: `assert_fs_read_to_string_le!(a_path, b_path)`\n",
...
macro_rules! debug_assert_fs_read_to_string_le {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_fs_read_to_string_le!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_fs_read_to_string_le {
    #[allow(unused_imports)]
    use std::io::Read;
    use std::panic;
    use std::path::PathBuf;
    ...
    fn lt() {
        ...
        let b = DIR.join("bravo.txt");
        for _ in 0..1 {
            let _actual = debug_assert_fs_read_to_string_le!(a, b);
            // assert_eq!(actual, (String::from("alfa\n"), String::from("bravo\n")));
        }
    }
