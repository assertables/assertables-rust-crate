...
#[cfg(test)]
mod test_assert_approx_ne_with_absolute_error_as_result {
    use std::sync::Once;
    ...
    fn success() {
        ...
        let delta: i8 = 1;
        for _ in 0..1 {
            let actual = assert_approx_ne_with_absolute_error_as_result!(a, b, delta);
    ...
    fn success_once() {
        ...
        let result = assert_approx_ne_with_absolute_error_as_result!(a(), b(), delta());
        assert!(result.is_ok());
...
macro_rules! assert_approx_ne_with_absolute_error {
    ($a:expr, $b:expr, $delta:expr $(,)?) => {
        match $crate::assert_approx_ne_with_absolute_error_as_result!($a, $b, $delta) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    };
...
#[cfg(test)]
mod test_assert_approx_ne_with_absolute_error {
    use std::panic;
    ...
    fn success() {
        ...
        let delta: i8 = 1;
        for _ in 0..1 {
            let actual = assert_approx_ne_with_absolute_error!(a, b, delta);
    ...
    fn failure() {
        ...
        let delta: i8 = 1;
        let result = panic::catch_unwind(|| {
            let _actual = assert_approx_ne_with_absolute_error!(a, b, delta);
        });
        let message = concat!(
            "assertion failed: `assert_approx_ne_with_absolute_error!(a, b, Δ)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_approx_ne_with_absolute_error.html\n",
...
macro_rules! debug_assert_approx_ne_with_absolute_error {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_approx_ne_with_absolute_error!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_approx_ne_with_absolute_error {
    use std::panic;
    ...
    fn success() {
        ...
        let delta: i8 = 1;
        for _ in 0..1 {
            let _actual = debug_assert_approx_ne_with_absolute_error!(a, b, delta);
            // assert_eq!(actual, (2 as i8, 1 as i8));
        }
    }
