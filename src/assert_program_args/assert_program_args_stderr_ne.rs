...
#[cfg(test)]
mod test_assert_program_args_stderr_ne_as_result {
    use std::sync::Once;
    ...
    fn lt() {
        ...
        let b_program = "bin/printf-stderr";
        let b_args = ["%s", "zz"];
        let actual = assert_program_args_stderr_ne_as_result!(a_program, a_args, b_program, b_args);
        assert_eq!(
            actual.unwrap(),
    ...
    fn lt_once() {
        ...
        let result = assert_program_args_stderr_ne_as_result!(a(), a_args(), b(), b_args());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_program_args_stderr_ne {
    use std::panic;
...
macro_rules! debug_assert_program_args_stderr_ne {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_program_args_stderr_ne!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_program_args_stderr_ne {
    use std::panic;
    ...
    fn lt() {
        ...
        for _ in 0..1 {
            let _actual =
                debug_assert_program_args_stderr_ne!(a_program, a_args, b_program, b_args);
            // assert_eq!(actual, (vec![b'a', b'l', b'f', b'a'], vec![b'z', b'z']));
        }
    }
