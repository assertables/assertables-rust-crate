...
#[cfg(test)]
mod test_assert_f64_gt_as_result {
    ...
    fn gt_once() {
        ...
        let result = assert_f64_gt_as_result!(a(), b());
        assert!(result.is_ok());
...
macro_rules! assert_f64_gt {
    ($a:expr, $b:expr $(,)?) => {
        match $crate::assert_f64_gt_as_result!($a, $b) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    };
