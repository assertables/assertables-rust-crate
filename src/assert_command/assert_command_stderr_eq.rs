...
#[cfg(test)]
mod test_assert_command_stderr_eq {
    use std::panic;
    use std::process::Command;
    ...
    fn lt() {
        let result = panic::catch_unwind(|| {
            ...
            let mut b = Command::new("bin/printf-stderr");
            b.args(["%s", "zz"]);
            let _actual = assert_command_stderr_eq!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_command_stderr_eq!(a_command, b_command)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_command_stderr_eq.html\n",
...
#[cfg(test)]
mod test_debug_assert_command_stderr_eq {
    use std::panic;
    use std::process::Command;
    ...
    fn eq() {
        ...
        b.args(["%s", "alfa"]);
        for _ in 0..1 {
            let _actual = debug_assert_command_stderr_eq!(a, b);
