...
#[cfg(test)]
mod test_assert_program_args_stderr_le_x_as_result {
    use std::sync::Once;
    ...
    fn lt() {
        ...
        let b = vec![b'z', b'z'];
        for _ in 0..1 {
            let actual = assert_program_args_stderr_le_x_as_result!(a_program, a_args, b);
    ...
    fn lt_once() {
        ...
        let result = assert_program_args_stderr_le_x_as_result!(a(), a_args(), b());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_program_args_stderr_le_x {
    use std::panic;
    ...
    fn lt() {
        ...
        let b = vec![b'z', b'z'];
        for _ in 0..1 {
            let actual = assert_program_args_stderr_le_x!(a_program, a_args, b);
    ...
    fn gt() {
        ...
        let b = vec![b'a', b'a'];
        let result = panic::catch_unwind(|| {
            let _actual = assert_program_args_stderr_le_x!(a_program, a_args, b);
        });
        let message = concat!(
            "assertion failed: `assert_program_args_stderr_le_x!(a_program, a_args, b_expr)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_program_args_stderr_le_x.html\n",
...
macro_rules! debug_assert_program_args_stderr_le_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_program_args_stderr_le_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_program_args_stderr_le_x {
    use std::panic;
    ...
    fn lt() {
        ...
        let b = vec![b'z', b'z'];
        for _ in 0..1 {
            let _actual = debug_assert_program_args_stderr_le_x!(a_program, a_args, b);
            // assert_eq!(actual, vec![b'a', b'l', b'f', b'a']);
        }
    }
