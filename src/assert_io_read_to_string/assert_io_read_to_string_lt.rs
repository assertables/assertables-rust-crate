...
#[cfg(test)]
mod test_assert_io_read_to_string_lt_as_result {
    ...
    fn lt() {
        let a = "alfa".as_bytes();
        let b = "zz".as_bytes();
        let actual = assert_io_read_to_string_lt_as_result!(a, b);
    ...
    fn lt_once() {
        ...
        let result = assert_io_read_to_string_lt_as_result!(a(), b());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_io_read_to_string_lt {
    #[allow(unused_imports)]
    use std::panic;
    ...
    fn success() {
        ...
        let b = "zz".as_bytes();
        for _ in 0..1 {
            let actual = assert_io_read_to_string_lt!(a, b);
    ...
    fn failure() {
        let result = panic::catch_unwind(|| {
            let a = "alfa".as_bytes();
            let b = "aa".as_bytes();
            let _actual = assert_io_read_to_string_lt!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_io_read_to_string_lt!(a_reader, b_reader)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_io_read_to_string_lt.html\n",
...
macro_rules! debug_assert_io_read_to_string_lt {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_io_read_to_string_lt!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_io_read_to_string_lt {
    #[allow(unused_imports)]
    use std::panic;
    ...
    fn success() {
        ...
        let b = "zz".as_bytes();
        for _ in 0..1 {
            let _actual = debug_assert_io_read_to_string_lt!(a, b);
            // assert_eq!(actual, (String::from("alfa"), String::from("zz")));
        }
    }
