...
#[cfg(test)]
mod test_assert_diff_le_x_as_result {
    use std::sync::Once;
    ...
    fn lt_once() {
        ...
        let result = assert_diff_le_x_as_result!(a(), b(), x());
        assert!(result.is_ok());
...
macro_rules! assert_diff_le_x {
    ($a:expr, $b:expr, $x:expr $(,)?) => {
        match $crate::assert_diff_le_x_as_result!($a, $b, $x) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    };
...
mod test_assert_diff_le_x {
    ...
    fn lt() {
        ...
        let x: i8 = 4;
        for _ in 0..1 {
            let actual = assert_diff_le_x!(a, b, x);
    ...
    fn gt() {
        ...
        let x: i8 = 2;
        let result = panic::catch_unwind(|| {
            let _actual = assert_diff_le_x!(a, b, x);
        });
        let message = concat!(
            "assertion failed: `assert_diff_le_x!(a, b, x)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_diff_le_x.html\n",
...
macro_rules! debug_assert_diff_le_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_diff_le_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_diff_le_x {
    use std::panic;
    ...
    fn lt() {
        ...
        let x: i8 = 4;
        for _ in 0..1 {
            let _actual = debug_assert_diff_le_x!(a, b, x);
            // assert_eq!(actual, (3 as i8, 4 as i8));
        }
    }
