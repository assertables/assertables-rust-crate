...
mod test_assert_ready_ne_as_result {
    ...
    fn ne() {
        ...
        let b: Poll<i8> = Ready(2);
        for _ in 0..1 {
            let actual = assert_ready_ne_as_result!(a, b);
...
mod test_assert_ready_ne {
    ...
    fn ne() {
        ...
        let b: Poll<i8> = Ready(2);
        for _ in 0..1 {
            let actual = assert_ready_ne!(a, b);
    ...
    fn eq() {
        ...
        let b: Poll<i8> = Ready(1);
        let result = panic::catch_unwind(|| {
            let _actual = assert_ready_ne!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_ready_ne!(a, b)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_ready_ne.html\n",
...
#[cfg(test)]
mod test_debug_assert_ready_ne {
    use std::panic;
    use std::task::Poll;
    use std::task::Poll::*;
    ...
    fn ne() {
        ...
        let b: Poll<i8> = Ready(2);
        for _ in 0..1 {
            let _actual = debug_assert_ready_ne!(a, b);
            // assert_eq!(actual, (1, 2));
        }
    }
