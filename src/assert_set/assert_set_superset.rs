...
#[cfg(test)]
mod test_assert_set_superset_as_result {
...
mod test_assert_set_superset {
    ...
    fn success() {
        let a = [1, 2, 3];
        let b = [1, 2];
        let actual = assert_set_superset!(a, b);
        assert_eq!(
            actual,
            (BTreeSet::from([&1, &2, &3]), BTreeSet::from([&1, &2]))
        );
    ...
    fn failure() {
        ...
        let b = [1, 2, 3];
        let result = panic::catch_unwind(|| {
            let _actual = assert_set_superset!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_set_superset!(a_collection, b_collection)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_set_superset.html\n",
...
#[cfg(test)]
mod test_debug_assert_set_superset {
    use std::collections::BTreeSet;
    use std::panic;
    ...
    fn success() {
        let a = [1, 2, 3];
        let b = [1, 2];
        let _actual = debug_assert_set_superset!(a, b);
        let _expect = (BTreeSet::from([&1, &2, &3]), BTreeSet::from([&1, &2]));
