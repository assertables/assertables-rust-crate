...
#[cfg(test)]
mod test_assert_command_stderr_eq_x_as_result {
    use std::process::Command;
    use std::sync::Once;
    ...
    fn eq() {
        ...
        for _ in 0..1 {
            let actual = assert_command_stderr_eq_x_as_result!(a, b);
    ...
    fn eq_once() {
        ...
        let result = assert_command_stderr_eq_x_as_result!(a(), b());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_command_stderr_eq_x {
    use std::panic;
    use std::process::Command;
    ...
    fn eq() {
        ...
        let b: Vec<u8> = vec![b'a', b'l', b'f', b'a'];
        for _ in 0..1 {
            let actual = assert_command_stderr_eq_x!(a, b);
    ...
    fn lt() {
        let result = panic::catch_unwind(|| {
            ...
            a.args(["%s", "alfa"]);
            let b = vec![b'a', b'a'];
            let _actual = assert_command_stderr_eq_x!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_command_stderr_eq_x!(command, expr)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_command_stderr_eq_x.html\n",
...
#[cfg(test)]
mod test_debug_assert_command_stderr_eq_x {
    use std::panic;
    use std::process::Command;
    ...
    fn eq() {
        ...
        let b: Vec<u8> = vec![b'a', b'l', b'f', b'a'];
        for _ in 0..1 {
            let _actual = debug_assert_command_stderr_eq_x!(a, b);
            // assert_eq!(actual, vec![b'a', b'l', b'f', b'a']);
        }
    }
