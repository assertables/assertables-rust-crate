...
#[cfg(test)]
mod test_assert_program_args_stdout_eq_x_as_result {
    use std::sync::Once;
    ...
    fn eq_once() {
        ...
        let result = assert_program_args_stdout_eq_x_as_result!(a(), a_args(), b());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_program_args_stdout_eq_x {
    use std::panic;
    ...
    fn eq() {
        ...
        let b = vec![b'a', b'l', b'f', b'a'];
        for _ in 0..1 {
            let actual = assert_program_args_stdout_eq_x!(a_program, a_args, b);
    ...
    fn lt() {
        ...
        let b = vec![b'z', b'z'];
        let result = panic::catch_unwind(|| {
            let _actual = assert_program_args_stdout_eq_x!(a_program, a_args, b);
        });
        let message = concat!(
            "assertion failed: `assert_program_args_stdout_eq_x!(a_program, a_args, b_expr)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_program_args_stdout_eq_x.html\n",
...
macro_rules! debug_assert_program_args_stdout_eq_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_program_args_stdout_eq_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_program_args_stdout_eq_x {
    use std::panic;
    ...
    fn eq() {
        ...
        let b = vec![b'a', b'l', b'f', b'a'];
        for _ in 0..1 {
            let _actual = debug_assert_program_args_stdout_eq_x!(a_program, a_args, b);
            // assert_eq!(actual, vec![b'a', b'l', b'f', b'a']);
        }
    }
