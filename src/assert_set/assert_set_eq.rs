...
#[cfg(test)]
mod test_assert_set_eq_as_result {
    ...
    fn success() {
        ...
        assert_eq!(
            actual.unwrap(),
            (BTreeSet::from([&1, &2]), BTreeSet::from([&1, &2]))
        );
    }
...
mod test_assert_set_eq {
    ...
    fn success() {
        ...
        let b = [1, 2];
        for _ in 0..1 {
            let actual = assert_set_eq!(a, b);
            assert_eq!(actual, (BTreeSet::from([&1, &2]), BTreeSet::from([&1, &2])));
        }
    }
...
macro_rules! debug_assert_set_eq {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_set_eq!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_set_eq {
    use std::collections::BTreeSet;
    use std::panic;
    ...
    fn success() {
        ...
        let b = [1, 2];
        for _ in 0..1 {
            let _actual = debug_assert_set_eq!(a, b);
            let _expect = (BTreeSet::from([&1, &2]), BTreeSet::from([&1, &2]));
