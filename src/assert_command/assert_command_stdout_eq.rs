            ...
            (a, b) => {
                Err(
                    format!(
                        concat!(
                            "assertion failed: `assert_command_stdout_eq!(a_command, b_command)`\n",
                            "https://docs.rs/assertables/8.18/assert_command_stdout_eq/assertables/macro.assert_command_stdout_eq.html\n",
                            " a label: `{}`,\n",
                            " a debug: `{:?}`,\n",
                            " a value: `{:?}`,\n",
                            " b label: `{}`,\n",
...
#[cfg(test)]
mod test_assert_command_stdout_eq_as_result {
    use std::process::Command;
    use std::sync::Once;
...
#[cfg(test)]
mod test_assert_command_stdout_eq {
    use std::panic;
    use std::process::Command;
    ...
    fn eq() {
        ...
        b.args(["%s", "alfa"]);
        for _ in 0..1 {
            let actual = assert_command_stdout_eq!(a, b);
            let expect = (vec![b'a', b'l', b'f', b'a'], vec![b'a', b'l', b'f', b'a']);
    ...
    fn lt() {
        let result = panic::catch_unwind(|| {
            ...
            let mut b = Command::new("bin/printf-stdout");
            b.args(["%s", "zz"]);
            let _actual = assert_command_stdout_eq!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_command_stdout_eq!(a_command, b_command)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_command_stdout_eq.html\n",
...
#[cfg(test)]
mod test_debug_assert_command_stdout_eq {
    use std::panic;
    use std::process::Command;
    ...
    fn eq() {
        ...
        b.args(["%s", "alfa"]);
        for _ in 0..1 {
            let _actual = debug_assert_command_stdout_eq!(a, b);
            // assert_eq!(
            //     actual,
            //     (vec![b'a', b'l', b'f', b'a'], vec![b'a', b'l', b'f', b'a'])
            // );
