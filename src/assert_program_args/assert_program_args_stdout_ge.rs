...
#[cfg(test)]
mod test_assert_program_args_stdout_ge_as_result {
    use std::sync::Once;
    ...
    fn gt() {
        ...
        let b_program = "bin/printf-stdout";
        let b_args = ["%s%s", "a", "a"];
        let actual = assert_program_args_stdout_ge_as_result!(a_program, a_args, b_program, b_args);
        assert_eq!(
            actual.unwrap(),
    ...
    fn gt_once() {
        ...
        let result = assert_program_args_stdout_ge_as_result!(a(), a_args(), b(), b_args());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_program_args_stdout_ge {
    use std::panic;
    ...
    fn gt() {
        ...
        let b_args = ["%s%s", "a", "a"];
        for _ in 0..1 {
            let actual = assert_program_args_stdout_ge!(a_program, a_args, b_program, b_args);
    ...
    fn lt() {
        ...
        let b_args = ["%s%s", "z", "z"];
        let result = panic::catch_unwind(|| {
            let _actual = assert_program_args_stdout_ge!(a_program, a_args, b_program, b_args);
        });
        let message: &str = concat!(
            "assertion failed: `assert_program_args_stdout_ge!(a_program, a_args, b_program, b_args)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_program_args_stdout_ge.html\n",
...
#[cfg(test)]
mod test_debug_assert_program_args_stdout_ge {
    use std::panic;
    ...
    fn gt() {
        ...
        for _ in 0..1 {
            let _actual =
                debug_assert_program_args_stdout_ge!(a_program, a_args, b_program, b_args);
            // assert_eq!(actual, (vec![b'a', b'l', b'f', b'a'], vec![b'a', b'a']));
        }
    }
    ...
    fn eq() {
        ...
        let b_program = "bin/printf-stdout";
        let b_args = ["%s%s%s%s", "a", "l", "f", "a"];
        let _actual = debug_assert_program_args_stdout_ge!(a_program, a_args, b_program, b_args);
        let _expect = (vec![b'a', b'l', b'f', b'a'], vec![b'a', b'l', b'f', b'a']);
