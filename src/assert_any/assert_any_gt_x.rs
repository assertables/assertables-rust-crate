...
//! # Module macros
//!
//! * [`assert_any_gt_x`](macro@crate::assert_any_gt_x)
//! * [`assert_any_gt_x_as_result`](macro@crate::assert_any_gt_x_as_result)
//! * [`debug_assert_any_gt_x`](macro@crate::debug_assert_any_gt_x)
...
#[cfg(test)]
mod test_assert_any_gt_x_as_result {
    // use std::sync::Once;
...
macro_rules! assert_any_gt_x {
    ($iter:expr, $x:expr $(,)?) => {
        match $crate::assert_any_gt_x_as_result!($iter, $x) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    };
...
mod test_assert_any_gt_x {
    ...
    fn gt() {
        ...
        let b = 0;
        for _ in 0..1 {
            let actual = assert_any_gt_x!(a.iter(), b);
            assert_eq!(actual, ());
        }
    }
    ...
    fn eq() {
        ...
        let b = 1;
        let result = panic::catch_unwind(|| {
            let _actual = assert_any_gt_x!(a.iter(), b);
        });
        let message = concat!(
            "assertion failed: `assert_any_gt_x!(iter, x)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_any_gt_x.html\n",
...
/// iter ∃ > x
///
/// This macro provides the same statements as [`assert_any_gt_x`](macro.assert_any_gt_x.html),
...
///
#[macro_export]
macro_rules! debug_assert_any_gt_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_any_gt_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_any_gt_x {
    use std::panic;
    ...
    fn success() {
        ...
        let b = 0;
        for _ in 0..1 {
            let _actual = debug_assert_any_gt_x!(a.iter(), b);
            // assert_eq!(actual, ());
        }
    }
