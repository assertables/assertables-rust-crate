...
#[cfg(test)]
mod test_assert_io_read_to_string_le_x_as_result {
    ...
    fn lt_once() {
        ...
        let result = assert_io_read_to_string_le_x_as_result!(a(), b());
        assert!(result.is_ok());
...
macro_rules! debug_assert_io_read_to_string_le_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_io_read_to_string_le_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_io_read_to_string_le_x {
    #[allow(unused_imports)]
    use std::panic;
    ...
    fn lt() {
        ...
        let x = String::from("zz");
        for _ in 0..1 {
            let _actual = debug_assert_io_read_to_string_le_x!(reader, x);
            // assert_eq!(actual, String::from("alfa"));
        }
    }
