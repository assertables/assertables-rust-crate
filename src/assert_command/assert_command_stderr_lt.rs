...
#[cfg(test)]
mod test_assert_command_stderr_lt_as_result {
    use std::process::Command;
    use std::sync::Once;
...
#[cfg(test)]
mod test_assert_command_stderr_lt {
    use std::panic;
    use std::process::Command;
    ...
    fn lt() {
        ...
        b.args(["%s", "zz"]);
        for _ in 0..1 {
            let actual = assert_command_stderr_lt!(a, b);
    ...
    fn eq() {
        let result = panic::catch_unwind(|| {
            ...
            let mut b = Command::new("bin/printf-stderr");
            b.args(["%s", "alfa"]);
            let _actual = assert_command_stderr_lt!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_command_stderr_lt!(a_command, b_command)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_command_stderr_lt.html\n",
