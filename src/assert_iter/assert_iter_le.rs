...
#[cfg(test)]
mod test_assert_iter_le_as_result {
    // use std::sync::Once;
...
mod test_assert_iter_le {
    ...
    fn gt() {
        ...
        let b = [1, 2];
        let result = panic::catch_unwind(|| {
            let _actual = assert_iter_le!(&a, &b);
        });
        let message = concat!(
            "assertion failed: `assert_iter_le!(a_collection, b_collection)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_iter_le.html\n",
...
mod test_debug_assert_iter_le {
    ...
    fn lt() {
        ...
        let b = [3, 4];
        for _ in 0..1 {
            let _actual = debug_assert_iter_le!(&a, &b);
            // assert_eq!(actual, ());
        }
    }
