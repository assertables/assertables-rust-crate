...
#[cfg(test)]
mod test_assert_f32_ge_as_result {
    ...
    fn gt() {
        ...
        let b: f32 = LT;
        for _ in 0..1 {
            let actual = assert_f32_ge_as_result!(a, b);
    ...
    fn gt_once() {
        ...
        let result = assert_f32_ge_as_result!(a(), b());
        assert!(result.is_ok());
...
macro_rules! assert_f32_ge {
    ($a:expr, $b:expr $(,)?) => {
        match $crate::assert_f32_ge_as_result!($a, $b) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    };
...
macro_rules! debug_assert_f32_ge {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_f32_ge!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_f32_ge {
    use crate::assert_f32::{EQ, LT};
    use std::panic;
    ...
    fn gt() {
        ...
        let b: f32 = LT;
        for _ in 0..1 {
            let _actual = debug_assert_f32_ge!(a, b);
            // assert_eq!(actual, ());
        }
    }
}
