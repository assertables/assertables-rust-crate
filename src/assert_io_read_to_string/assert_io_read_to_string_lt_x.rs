...
mod test_assert_io_read_to_string_lt_x_as_result {
    ...
    fn lt() {
        ...
        let x = String::from("zz");
        for _ in 0..1 {
            let actual = assert_io_read_to_string_lt_x_as_result!(reader, x);
    ...
    fn lt_once() {
        ...
        let result = assert_io_read_to_string_lt_x_as_result!(a(), b());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_io_read_to_string_lt_x {
    #[allow(unused_imports)]
    use std::panic;
    ...
    fn lt() {
        ...
        let x = String::from("zz");
        for _ in 0..1 {
            let actual = assert_io_read_to_string_lt_x!(reader, x);
    ...
    fn eq() {
        let result = panic::catch_unwind(|| {
            let reader = "alfa".as_bytes();
            let x = String::from("alfa");
            let _actual = assert_io_read_to_string_lt_x!(reader, x);
        });
        let message = concat!(
            "assertion failed: `assert_io_read_to_string_lt_x!(a_reader, b_expr)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_io_read_to_string_lt_x.html\n",
...
macro_rules! debug_assert_io_read_to_string_lt_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_io_read_to_string_lt_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_io_read_to_string_lt_x {
    #[allow(unused_imports)]
    use std::panic;
    ...
    fn lt() {
        ...
        let x = String::from("zz");
        for _ in 0..1 {
            let _actual = debug_assert_io_read_to_string_lt_x!(reader, x);
            // assert_eq!(actual, String::from("alfa"));
        }
    }
