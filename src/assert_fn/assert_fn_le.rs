...
mod test_debug_assert_fn_le {
    ...
    mod arity_1 {
        ...
        fn lt() {
            ...
            let b: i8 = 2;
            for _ in 0..1 {
                let _actual = debug_assert_fn_le!(f, a, g, b);
                let _expect = (1, 2);
                // assert_eq!(actual, expect);
            }
        }
    ...
    mod arity_0 {
        ...
        fn lt() {
            for _ in 0..1 {
                let _actual = debug_assert_fn_le!(f, g);
                let _expect = (1, 2);
                // assert_eq!(actual, expect);
            }
        }
        ...
        fn eq() {
            for _ in 0..1 {
                let _actual = debug_assert_fn_le!(f, f);
                let _expect = (1, 1);
                // assert_eq!(actual, expect);
            }
        }
        ...
        fn gt() {
            let result = panic::catch_unwind(|| {
                let _actual = debug_assert_fn_le!(g, f);
            });
            let message = concat!(
                "assertion failed: `assert_fn_le!(a_function, b_function)`\n",
                "https://docs.rs/assertables/10.0.0/assertables/macro.assert_fn_le.html\n",
