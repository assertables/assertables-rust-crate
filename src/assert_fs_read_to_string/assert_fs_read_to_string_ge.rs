...
mod test_assert_fs_read_to_string_ge_as_result {
    ...
    fn gt() {
        ...
        let actual = assert_fs_read_to_string_ge_as_result!(a, b);
        assert_eq!(
            actual.unwrap(),
            (String::from("bravo\n"), String::from("alfa\n"))
        );
...
#[cfg(test)]
mod test_assert_fs_read_to_string_ge {
    #[allow(unused_imports)]
    use std::io::Read;
    use std::panic;
    use std::path::PathBuf;
    ...
    fn lt() {
        ...
        let b = DIR.join("bravo.txt");
        let result = panic::catch_unwind(|| {
            let _actual = assert_fs_read_to_string_ge!(a, b);
        });
        let message = format!(
            concat!(
                "assertion failed: `assert_fs_read_to_string_ge!(a_path, b_path)`\n",
...
macro_rules! debug_assert_fs_read_to_string_ge {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_fs_read_to_string_ge!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_fs_read_to_string_ge {
    #[allow(unused_imports)]
    use std::io::Read;
    use std::panic;
    use std::path::PathBuf;
    ...
    fn gt() {
        ...
        let b = DIR.join("alfa.txt");
        for _ in 0..1 {
            let _actual = debug_assert_fs_read_to_string_ge!(a, b);
            // assert_eq!(actual, (String::from("bravo\n"), String::from("alfa\n")));
        }
    }
