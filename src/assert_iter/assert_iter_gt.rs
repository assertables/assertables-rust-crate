...
#[cfg(test)]
mod test_assert_iter_gt_as_result {
    // use std::sync::Once;
...
mod test_assert_iter_gt {
    ...
    fn eq() {
        ...
        let b = [1, 2];
        let result = panic::catch_unwind(|| {
            let _actual = assert_iter_gt!(&a, &b);
        });
        let message = concat!(
            "assertion failed: `assert_iter_gt!(a_collection, b_collection)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_iter_gt.html\n",
...
mod test_debug_assert_iter_gt {
    ...
    fn gt() {
        ...
        let b = [1, 2];
        for _ in 0..1 {
            let _actual = debug_assert_iter_gt!(&a, &b);
            // assert_eq!(actual, ());
        }
    }
