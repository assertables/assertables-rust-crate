...
#[cfg(test)]
mod test_assert_command_stderr_le_as_result {
    use std::process::Command;
    use std::sync::Once;
...
#[cfg(test)]
mod test_assert_command_stderr_le {
    use std::panic;
    use std::process::Command;
    ...
    fn lt() {
        ...
        b.args(["%s", "zz"]);
        for _ in 0..1 {
            let actual = assert_command_stderr_le!(a, b);
    ...
    fn gt() {
        let result = panic::catch_unwind(|| {
            ...
            let mut b = Command::new("bin/printf-stderr");
            b.args(["%s", "aa"]);
            let _actual = assert_command_stderr_le!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_command_stderr_le!(a_command, b_command)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_command_stderr_le.html\n",
...
#[cfg(test)]
mod test_debug_assert_command_stderr_le {
    use std::panic;
    use std::process::Command;
    ...
    fn lt() {
        ...
        b.args(["%s", "zz"]);
        for _ in 0..1 {
            let _actual = debug_assert_command_stderr_le!(a, b);
            // assert_eq!(actual, (vec![b'a', b'l', b'f', b'a'], vec![b'z', b'z']));
        }
    }
