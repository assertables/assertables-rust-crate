...
#[cfg(test)]
mod test_assert_diff_eq_x_as_result {
    use std::sync::Once;
...
macro_rules! assert_diff_eq_x {
    ($a:expr, $b:expr, $x:expr $(,)?) => {
        match $crate::assert_diff_eq_x_as_result!($a, $b, $x) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    };
...
mod test_assert_diff_eq_x {
    ...
    fn eq() {
        ...
        let x: i8 = 3;
        for _ in 0..1 {
            let actual = assert_diff_eq_x!(a, b, x);
    ...
    fn lt() {
        ...
        let x: i8 = 4;
        let result = panic::catch_unwind(|| {
            let _actual = assert_diff_eq_x!(a, b, x);
        });
        let message = concat!(
            "assertion failed: `assert_diff_eq_x!(a, b, x)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_diff_eq_x.html\n",
...
#[cfg(test)]
mod test_debug_assert_diff_eq_x {
    use std::panic;
    ...
    fn eq() {
        ...
        let x: i8 = 3;
        for _ in 0..1 {
            let _actual = debug_assert_diff_eq_x!(a, b, x);
            // assert_eq!(actual, (3 as i8, 3 as i8));
        }
    }
