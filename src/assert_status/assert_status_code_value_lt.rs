...
#[cfg(test)]
mod test_assert_status_code_value_lt_as_result {
    use std::process::Command;
    use std::sync::Once;
...
#[cfg(test)]
mod test_assert_status_code_value_lt {
    use std::panic;
    use std::process::Command;
    ...
    fn lt() {
        ...
        b.arg("2");
        for _ in 0..1 {
            let actual = assert_status_code_value_lt!(a, b);
    ...
    fn eq() {
        let result = panic::catch_unwind(|| {
            ...
            let mut b = Command::new("bin/exit-with-arg");
            b.arg("1");
            let _actual = assert_status_code_value_lt!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_status_code_value_lt!(a, b)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_status_code_value_lt.html\n",
...
#[cfg(test)]
mod test_debug_assert_status_code_value_lt {
    use std::panic;
    use std::process::Command;
    ...
    fn lt() {
        ...
        b.arg("2");
        for _ in 0..1 {
            let _actual = debug_assert_status_code_value_lt!(a, b);
            // assert_eq!(actual, (1, 2));
        }
    }
