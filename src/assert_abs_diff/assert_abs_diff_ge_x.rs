...
#[cfg(test)]
mod test_assert_abs_diff_ge_x_as_result {
    use std::sync::Once;
    ...
    fn gt() {
        ...
        let x: i8 = 2;
        for _ in 0..1 {
            let actual = assert_abs_diff_ge_x_as_result!(a, b, x);
            let expect = (3, 2);
    ...
    fn gt_once() {
        ...
        let result = assert_abs_diff_ge_x_as_result!(a(), b(), x());
        assert!(result.is_ok());
...
macro_rules! assert_abs_diff_ge_x {
    ($a:expr, $b:expr, $x:expr $(,)?) => {
        match $crate::assert_abs_diff_ge_x_as_result!($a, $b, $x) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    };
...
mod test_assert_abs_diff_ge_x {
    ...
    fn gt() {
        ...
        let x = 2;
        for _ in 0..1 {
            let actual = assert_abs_diff_ge_x!(a, b, x);
            let expect = (3, 2);
    ...
    fn lt() {
        let result = panic::catch_unwind(|| {
            ...
            let b = 13;
            let x = 4;
            let _actual = assert_abs_diff_ge_x!(a, b, x);
        });
        let message = concat!(
            "assertion failed: `assert_abs_diff_ge_x!(a, b, x)`\n",
            "https://docs.rs/assertables/10.1.0/assertables/macro.assert_abs_diff_ge_x.html\n",
...
#[cfg(test)]
mod test_debug_assert_abs_diff_ge_x {
    use std::panic;
    ...
    fn gt() {
        ...
        let x = 2;
        for _ in 0..1 {
            let _actual = debug_assert_abs_diff_ge_x!(a, b, x);
            let _expect = (3, 2);
            // assert_eq!(actual, expect);
        }
    }
