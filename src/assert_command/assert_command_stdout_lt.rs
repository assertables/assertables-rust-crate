...
#[cfg(test)]
mod test_assert_command_stdout_lt_as_result {
    use std::process::Command;
    use std::sync::Once;
    ...
    fn lt_once() {
        ...
        let result = assert_command_stdout_lt_as_result!(a(), b());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_command_stdout_lt {
    use std::panic;
    use std::process::Command;
    ...
    fn lt() {
        ...
        b.args(["%s", "zz"]);
        for _ in 0..1 {
            let actual = assert_command_stdout_lt!(a, b);
    ...
    fn eq() {
        let result = panic::catch_unwind(|| {
            ...
            let mut b = Command::new("bin/printf-stdout");
            b.args(["%s", "alfa"]);
            let _actual = assert_command_stdout_lt!(a, b);
        });
        let message = concat!(
            "assertion failed: `assert_command_stdout_lt!(a_command, b_command)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_command_stdout_lt.html\n",
...
macro_rules! debug_assert_command_stdout_lt {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_command_stdout_lt!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_command_stdout_lt {
    use std::panic;
    use std::process::Command;
    ...
    fn lt() {
        ...
        b.args(["%s", "zz"]);
        for _ in 0..1 {
            let _actual = debug_assert_command_stdout_lt!(a, b);
            // assert_eq!(actual, (vec![b'a', b'l', b'f', b'a'], vec![b'z', b'z']));
        }
    }
