...
#[cfg(test)]
mod test_assert_count_ge_x_as_result {
    use std::sync::Once;
    ...
    fn gt_once() {
        ...
        let result = assert_count_ge_x_as_result!(a(), b());
        assert!(result.is_ok());
...
macro_rules! debug_assert_count_ge_x {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_count_ge_x!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_count_ge_x {
    use std::panic;
    ...
    fn gt() {
        ...
        let b = 1;
        for _ in 0..1 {
            let _actual = debug_assert_count_ge_x!(a, b);
            // assert_eq!(actual, (2, 1));
        }
    }
