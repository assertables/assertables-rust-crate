...
#[cfg(test)]
mod test_assert_program_args_stdout_gt_as_result {
    use std::sync::Once;
    ...
    fn gt() {
        ...
        let b_program = "bin/printf-stdout";
        let b_args = ["%s%s", "a", "a"];
        let actual = assert_program_args_stdout_gt_as_result!(a_program, a_args, b_program, b_args);
        assert_eq!(
            actual.unwrap(),
...
#[cfg(test)]
mod test_assert_program_args_stdout_gt {
    use std::panic;
...
#[cfg(test)]
mod test_debug_assert_program_args_stdout_gt {
    use std::panic;
    ...
    fn gt() {
        ...
        for _ in 0..1 {
            let _actual =
                debug_assert_program_args_stdout_gt!(a_program, a_args, b_program, b_args);
            // assert_eq!(actual, (vec![b'a', b'l', b'f', b'a'], vec![b'a', b'a']));
        }
    }
