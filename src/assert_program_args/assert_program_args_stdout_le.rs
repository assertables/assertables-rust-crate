...
#[cfg(test)]
mod test_assert_program_args_stdout_le_as_result {
    use std::sync::Once;
    ...
    fn lt_once() {
        ...
        let result = assert_program_args_stdout_le_as_result!(a(), a_args(), b(), b_args());
        assert!(result.is_ok());
...
#[cfg(test)]
mod test_assert_program_args_stdout_le {
    use std::panic;
    ...
    fn lt() {
        ...
        let b_args = ["%s%s", "z", "z"];
        for _ in 0..1 {
            let actual = assert_program_args_stdout_le!(a_program, a_args, b_program, b_args);
    ...
    fn gt() {
        ...
        let b_args = ["%s%s", "a", "a"];
        let result = panic::catch_unwind(|| {
            let _actual = assert_program_args_stdout_le!(a_program, a_args, b_program, b_args);
        });
        let message = concat!(
            "assertion failed: `assert_program_args_stdout_le!(a_program, a_args, b_program, b_args)`\n",
            "https://docs.rs/assertables/10.0.0/assertables/macro.assert_program_args_stdout_le.html\n",
...
#[cfg(test)]
mod test_debug_assert_program_args_stdout_le {
    use std::panic;
    ...
    fn lt() {
        ...
        for _ in 0..1 {
            let _actual =
                debug_assert_program_args_stdout_le!(a_program, a_args, b_program, b_args);
            // assert_eq!(actual, (vec![b'a', b'l', b'f', b'a'], vec![b'z', b'z']));
        }
    }
    ...
    fn eq() {
        ...
        let b_program = "bin/printf-stdout";
        let b_args = ["%s%s%s%s", "a", "l", "f", "a"];
        let _actual = debug_assert_program_args_stdout_le!(a_program, a_args, b_program, b_args);
