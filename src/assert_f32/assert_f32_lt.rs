...
#[cfg(test)]
mod test_assert_f32_lt_as_result {
    ...
    fn lt() {
        ...
        let b: f32 = GT;
        for _ in 0..1 {
            let actual = assert_f32_lt_as_result!(a, b);
    ...
    fn lt_once() {
        ...
        let result = assert_f32_lt_as_result!(a(), b());
        assert!(result.is_ok());
...
macro_rules! assert_f32_lt {
    ($a:expr, $b:expr $(,)?) => {
        match $crate::assert_f32_lt_as_result!($a, $b) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    };
...
macro_rules! debug_assert_f32_lt {
    ($($arg:tt)*) => {
        if cfg!(debug_assertions) {
            $crate::assert_f32_lt!($($arg)*);
        }
    };
}

#[cfg(test)]
mod test_debug_assert_f32_lt {
    use crate::assert_f32::{EQ, GT};
    use std::panic;
    ...
    fn lt() {
        ...
        let b: f32 = GT;
        for _ in 0..1 {
            let _actual = debug_assert_f32_lt!(a, b);
            // assert_eq!(actual, ());
        }
    }
}
